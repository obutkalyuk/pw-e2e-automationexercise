...
test.describe('Payment Idempotency Coverage', () => {
  test('[API-23][TR-14] Repeated payment submit in one session should preserve consistent completion artifacts @high', async ({ request, managedUser }, testInfo) => {
    ...
    const firstSubmit = await apiHelper.submitPaymentWithCsrfTokenRaw(request, paymentForm.csrfToken!);
    const secondSubmit = await apiHelper.submitPaymentWithCsrfTokenRaw(request, paymentForm.csrfToken!);

    const uniquePaymentArtifactIds = [...new Set([firstSubmit.paymentArtifactId, secondSubmit.paymentArtifactId].filter((value): value is string => Boolean(value)))];
    ...
    for (const paymentArtifactId of uniquePaymentArtifactIds) {
      const paymentDone = await apiHelper.captureTransportResponse(request, `/payment_done/${paymentArtifactId}`);
      const invoice = await apiHelper.captureTransportResponse(request, `/download_invoice/${paymentArtifactId}`);
