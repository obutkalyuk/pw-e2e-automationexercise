...
| Covered (Known defect) | API-26 | Deleted account session should not access checkout/payment    | Hybrid     | Verify stale authenticated session is rejected after account deletion via API          | Critical     | Known defect `#46`: deleted account can continue checkout/payment from stale session                             | tests\api\chain\account-deleted-session.chain.api.spec.ts |
...
| Covered (Known defect) | E2E-28 | Deleted account cannot complete checkout   | Orders / Auth  | E2E                 | Known defect `#46`: stale UI session can continue checkout after account deletion via API | tests\e2e\checkout-deleted-account.spec.ts |
...
| Covered (Known defect) | E2E-10-neg | Verify Subscription in Home Page submits real network request         | Pages / Subscription | E2E                 | Known defect `BUG-43` / BF-002: success message shown without actual network request                                | tests\e2e\subscription-footer.spec.ts |
| Covered (Known defect) | E2E-11-neg | Verify Subscription in Cart Page submits real network request         | Pages / Subscription | E2E                 | Known defect `BUG-43` / BF-002: success message shown without actual network request                                | tests\e2e\subscription-footer.spec.ts |
