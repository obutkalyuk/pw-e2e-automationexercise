...
|----------| ----- | ---------------- | ---------- | ------------------------------- | ------------ | ------------------------- | --------------|
|✅| API-1 | `/createAccount` | POST       | Create user account             | **Critical** | Base for all hybrid tests |tests\api\auth\account_flow.api.spec.ts|
|✅| API-2 | `/deleteAccount` | DELETE     | Delete user account             | **Critical** | Cleanup                   |tests\api\auth\account_flow.api.spec.ts|
|✅| API-3 | `/verifyLogin`   | POST       | Verify login with valid details | **Critical** | authorization check       |tests\api\auth\account_flow.api.spec.ts|

### High Priority (Regression API)

|**Status**| **#** | **API Endpoint**        | **Method** | **Description**         | **Priority** | **Notes**               | **File Name** |
|------| ----- | ----------------------- | ---------- | ----------------------- | ------------ | ----------------------- |---------------|
|✅| API-4 | `/productsList`         | GET        | Get all products list   | **High**     | check JSON products     |tests\api\catalog\catalog.api.spec.ts|
|✅| API-5 | `/searchProduct`        | POST       | Search for a product    | **High**     | search logic validation |tests\api\catalog\catalog.api.spec.ts|
|✅| API-6 | `/getUserDetailByEmail` | GET        | Get user account detail | **High**     |                         |tests\api\auth\account_flow.api.spec.ts|
